...
export function SignInLayout({
  ...
  //   if (form.formState.errors.password1?.type === "min_length") {
  //     alertDispatch({
  //       type: "add",
  ...
  async function onSubmit(data: IFormInput, e: BaseSyntheticEvent | undefined) {
    ...
    try {
      ...
      // get an access token so we don't need to do it for awhile
      ...
      //     type: "add",
      //     alert: makeErrorAlert({
      //       message: "Access token not granted.",
      //     }),
      //   })
      ...
      // accountDispatch({ type: "set", account: await LoadAccount() })
                    ...
                    {/* <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-row items-center gap-x-2">
                              ...
                              <FormLabel>{TEXT_PASSWORDLESS}</FormLabel>
                            </FormItem>
                          )}
                        /> */}
                  </div>
  ...
  return (
    ...
    >
      ...
      {alwaysShowSignIn || (signInEnabled && user.publicId === '')
        ? renderSignIn()
        : children}
    </HeaderLayout>
  )
